...
class TestRA1572Power(TestCase):
    ...
    def test_toggle(self):
        power_dict = {True: "power=on", False: "power=standby"}
        for state in power_dict.items():
            with self.subTest(value=state[1]):
...
class TestRA1572Mute(TestCase):
    def setUp(self) -> None:
        self.amp = RA1572()
        self.mute_dict = {True: "on", False: "off"}
...
class TestRA1572AutoUpdate(TestCase):
    ...
    def test_returns_auto_or_manual(self):
        update_dict = {True: "auto", False: "manual"}
...
class TestRA1572HandleMessage(TestCase):
    ...
    def test_vol_up(self):
        ...
        self.assertEqual(self.amp._volume, 51)

    def test_mute_toggle(self):
        self.amp._mute = True
        self.amp.handle_message("mute!")
        self.assertFalse(self.amp._mute)
